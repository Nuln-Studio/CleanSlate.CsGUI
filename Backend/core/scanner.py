...
def scan_duplicate_files() -> Dict:
    ...
    backend_log("debug", " 分组完成")
    hashes = {}
    total_dup_size = 0
    backend_log("debug", " 计算哈希...")
    hash_count = 0
...
def get_all_scans() -> Dict[str, Dict]:
    ...
    for item in SCAN_ITEMS:
        ...
        if func:
            #backend_log("debug", f"正在扫描: {item['name']}...")
            print(f"[SCAN] 正在扫描: {item['name']}...")
            data = func()
            data['risk'] = item['risk']
            results[item['id']] = data
            #backend_log("debug", f" 完成 ({data.get('size_gb',0):.2f} GB)")
            print(f"[SCAN] 完成 ({data.get('size_gb',0):.2f} GB)")
    return results

from core.config import CUSTOM_CACHE_DIRS, ENABLE_RECYCLE_BIN
